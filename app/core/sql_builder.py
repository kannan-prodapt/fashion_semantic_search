...
def build_sql_from_filters(filters: Dict[str, Any], limit: int):
    ...
    for dim, (table, alias) in label_map.items():
        ...
        if isinstance(values_not_in, list) and values_not_in:
            ...
            other_params.extend(values_not_in)

    # Numeric filters (hard constraints)
    price_min = filters.get("price_min")
    price_max = filters.get("price_max")
    rating_min = filters.get("rating_min")

    if price_min is not None:
        other_where_clauses.append("p.price >= %s")
        other_params.append(float(price_min))

    if price_max is not None:
        other_where_clauses.append("p.price <= %s")
        other_params.append(float(price_max))

    if rating_min is not None:
        other_where_clauses.append("p.average_rating >= %s")
        other_params.append(float(rating_min))

    # Store filters: support single store, IN and NOT IN (hard constraints)
    ...
    if isinstance(store_in, list) and store_in:
        ...
        other_where_clauses.append(f"p.store IN ({placeholders})")
        other_params.extend(store_in)

    if isinstance(store_not_in, list) and store_not_in:
        placeholders = ", ".join(["%s"] * len(store_not_in))
        other_where_clauses.append(f"p.store NOT IN ({placeholders})")
        other_params.extend(store_not_in)
    ...
    if isinstance(store, str) and store.strip() and not store_in:
        ...
        other_where_clauses.append("p.store = %s")
        other_params.append(store.strip())

    # main_category filters: support single, IN, NOT IN (hard constraints)
    ...
    if isinstance(main_category_in, list) and main_category_in:
        ...
        other_where_clauses.append(f"p.main_category IN ({placeholders})")
        other_params.extend(main_category_in)

    if isinstance(main_category_not_in, list) and main_category_not_in:
        placeholders = ", ".join(["%s"] * len(main_category_not_in))
        other_where_clauses.append(f"p.main_category NOT IN ({placeholders})")
        other_params.extend(main_category_not_in)
    ...
    if isinstance(main_category, str) and main_category.strip() and not main_category_in:
        ...
        other_where_clauses.append("p.main_category = %s")
        other_params.append(main_category.strip())
    ...
    if other_where_clauses:
        where_clauses.extend(other_where_clauses)
